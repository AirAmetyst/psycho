...
c = 1
results = []
#answers = {"Не согласен частично": -2,"Согласен полностью": 3, "Скорее не согласен  чем согласен":-1,"Не согласен полностью":-3,"Скорее согласен  чем не согласен":1,"Согласен частично":2  }
...
for i in data[3:]:
    ...
    will = 0
    t = i.split(';')
    for j in range(184,223):
        if j-184 in [0,2,5,6,7,9,13,15,21,24,26,30,33,38]:
            goal += answers[t[j]]
        if j-184 in [1,4,6,10,15,16,17,22,27,31,32,35,36,37]:
            analysis += answers[t[j]]
        if j-184 in [2,11,17,22,23,26,27,28,33,35,36]:
            plan += answers[t[j]]
        if j-184 in [1,2,12,15,16,17,18,22,27,29,30,32,33,35,37]:
            selfcontrol += answers[t[j]]
        if j-184 in [4,8,13,14,20,24,25,26,31,34]:
            if j - 184 in [4,8,26,31,34]:
                correction += -1*answers[t[j]]
            else:
                correction += answers[t[j]]
        if j-184 in [3,5,6,7,13,17,19,26,30,38,26,30,33,38]:
            if j-184 in [26]:
                will += -1* answers[t[j]]
            else:
                will +=  answers[t[j]]
...
for i in data[3:]:
    ...
    will = 0
    t = i.split(';')
    for j in range(159, 185):

        
        if j - 159 in [2,3,7,10]:
            goal += answers[t[j]]
